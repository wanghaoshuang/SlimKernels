...
In our solution, the PTX instruction `mma.sync.aligned.m16n8k8.row.col.f32.tf32.tf32.f32` is employed to compute a 16X8 tile with FP32 elements. Because each loading transaction can only read 128bits X 32 = 128bytes, so we need four transanctions to load a tile with 16X8X4bytes = 512Bytes.

>Why each loading transaction read 128bits X 32 = 128bytes? A single vectorized access instruction can process up to 128 bits. 32 is the number of threads in a warp, and also the number of banks.
