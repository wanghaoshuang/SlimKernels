...
__device__ void mma_tile(MMAarguments &arg,ElementInputA *A,ElementInputB *B,ElementAccumulator *C,ElementOutput *D){
    ...
    for(int tileidx=0;tileidx<tile_size;tileidx++){
        ...
        int a_addr = (rowwarp * WARP_SHAPE_M + rowtile * TENSOR_CORE_M) * TENSOR_CORE_K + (laneidx / 8) * num_per_thread + (laneidx % 8) * TENSOR_CORE_N;
        ...
        asm volatile(
                ...
                "=r"(*(uint32_t*)&a[3])
                : 
                "r"((uint32_t)__cvta_generic_to_shared(&A[a_addr])) 
            );
